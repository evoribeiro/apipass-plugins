...
| "Method and URL are required, check your flow configuration." no step de acao (MEMORY_STORE_SET/GET, PROJECT_STORE_SET/GET, LOGGER etc.) — mesmo com `actionId`/`coreRouteType` corretos no step | Faltou `coreRouteType` no **objeto de link** dentro do array `nextSteps` do step anterior. O engine usa esse campo para resolver a URL do microsservico — sem ele nao roteia e lanca o erro. Causa silenciosa: `get_flow_development` e `save`/`publish` aceitam sem reclamar, mas a execucao falha | Em cada item de `nextSteps` que aponta para um `.service.actions.Action`, inclua `"coreRouteType"` com o mesmo valor do step de destino. Ex: `{"id":"a0","type":".service.actions.Action","coreRouteType":"MEMORY_STORE_GET","sourceUUID":"...","targetUUID":"..."}`. Vale para TODOS os acoes de catalogo. Leia um fluxo funcional existente com `get_flow_development` para confirmar o shape correto do link. |
| MEMORY_STORE_GET — valor vem `undefined` no NodeJS ou interpolacao retorna vazio | Usou `$.aN.body.value` assumindo que steps de catalogo retornam em `.body` — mas steps de catalogo (`.service.actions.Action`) NAO retornam em `.body`. Apenas HTTP, NodeJS e custom actions com `executeHttpRequest` retornam em `.body`. MEMORY_STORE_GET injeta o resultado em `$.aN.value` | Use `$.aN.value` no NodeJS e `{{$.aN.value}}` na interpolacao. Para outros steps de catalogo, consulte a doc da acao (`read_step_payload` num fluxo real) para descobrir o campo de output. |
| Switch roteia para Logger e step Logger falha com "Method and URL are required" | Link no `nextSteps` do step anterior usa `type: ".service.actions.Action"` + `coreRouteType: "LOGGER_UTILITY"` — shape de step, nao de link | O **link** (objeto em `nextSteps`) que aponta para um Logger deve usar `type: ".utility.logger.LoggerUtility"` (sem `coreRouteType`). O **step** Logger em si usa `type: ".service.actions.Action"`, `actionId: "LOGGER"`, `coreRouteType: "LOGGER_UTILITY"`, `authorization: ""`. Logger e o unico caso em que o `type` do link difere do `type` do step de destino. |
