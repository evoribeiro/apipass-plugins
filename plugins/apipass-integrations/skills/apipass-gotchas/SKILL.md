...
| Step NodeJS (`.utility.nodejs.NodeJSUtility`) trava e expira em ~2min (`"Execution timeout"`), sem nenhum erro no save | Codigo colocado em `rawData` em vez de `code` — `rawData` e o campo do `.service.http.HttpRequest`, mas e aceito silenciosamente no NodeJS tambem | Use SEMPRE `code` para o JS do NodeJS. `usedSteps` nao precisa ser preenchido a mao — o engine deriva automaticamente das referencias `$.aN` dentro do `code` |
...
| Filtro num dashboard/relatorio via `<form method="GET">` nunca da match quando o valor tem espaco (ex. filtro de cliente/categoria some com "0 resultados" mesmo o dado existindo) | `{{$.trigger.queryParams.*}}` preserva `+` literal em vez de decodificar como espaco — convencao de formulario GET codifica espaco como `+` na query string (`?cliente=Contoso+Seguros`), mas o RestTrigger nao desfaz isso | Decodifique manualmente antes de comparar/filtrar: `(v || '').replace(/\+/g, ' ')`. Vale para qualquer queryParam vindo de `<select>`/`<input>` de formulario, nao so nomes — categorias/status com espaco no proprio valor tambem sao afetados |
...
### Data Store — trava (hang) silenciosa e escopo de persistencia

| Sintoma | Causa provavel | Correcao |
|---|---|---|
| Execucao de um step `.service.actions.Action` de Data Store (PROJECT_STORE_GET/SET, ACCOUNT_STORE_GET/SET, MEMORY_STORE_GET/SET) fica `RUNNING` indefinidamente — sem erro, sem timeout, mesmo com `coreRouteType` presente no step, no link `nextSteps` e no `defaultStepCoreRouteType` do switch | O **valor** do `coreRouteType` esta errado (nao so ausente). Erro mais comum: acrescentar o sufixo `_UTILITY` (ex. `PROJECT_STORE_GET_UTILITY`, `ACCOUNT_STORE_SET_UTILITY`) — parece plausivel porque o `label` i18n da acao E `ACTIONS.DATA_STORE.PROJECT_STORE_GET_UTILITY`, mas o `coreRouteType` real e o **actionId puro, sem sufixo** (`PROJECT_STORE_GET`). Diferente do caso de `coreRouteType` AUSENTE (erro rapido "Method and URL are required"), o valor ERRADO-mas-plausivel faz o engine tentar resolver uma rota inexistente e travar sem nunca reportar erro — reproduzido de forma consistente em GET e SET, tanto em PROJECT_STORE quanto ACCOUNT_STORE | Use o `coreRouteType` EXATO devolvido por `get_action(groupId: "DATA-STORE", id: "<ACTION_ID>")` (campo `coreRouteType` da resposta) — sem sufixo algum. Confirme tambem lendo um fluxo funcional real com `get_flow_development`. Se uma execucao ja estiver travada, pare com `stop_execution` (confirme com o usuario) antes de corrigir e testar de novo. Sinal de que o valor esta certo: o log de step passa a mostrar um `type` nativo (`.store.ProjectStoreGetRouteConfig`/`SetRouteConfig`, `.store.MemoryStoreGetRouteConfig`/`SetRouteConfig`) e responde em milissegundos, em vez de ficar pendurado |
| Contador incremental (ex. `registration_number`) gravado via `MEMORY_STORE_SET` nao aparece na proxima execucao — `MEMORY_STORE_GET` sempre volta `null`, mesmo minutos depois, mesma chave exata | `MEMORY_STORE_*` e efemero — o valor NAO persiste entre execucoes separadas do fluxo (confirmado empiricamente: SET numa execucao, GET numa execucao diferente pouco depois retorna `null`). Adequado so para estado dentro de UMA execucao (ex. cursor de paginacao num loop, chave prefixada com `{{$.flowExecution.id}}`) | Para estado que precisa sobreviver entre execucoes diferentes (contadores, "ultimo valor processado", etc.), use `PROJECT_STORE_GET/SET` (persiste por projeto — confirmado empiricamente incrementando um contador em 3 execucoes seguidas) ou `ACCOUNT_STORE_GET/SET` (persiste por conta) — nunca `MEMORY_STORE` |
