...
| MEMORY_STORE_GET — valor vem `undefined` no NodeJS ou interpolacao retorna vazio | Usou `$.aN.body.value` seguindo a regra geral do `.body` — mas MEMORY_STORE_GET e a **unica excecao**: ele injeta o resultado diretamente como `$.aN.value` (sem wrapper `.body`) | Use `$.aN.value` no NodeJS e `{{$.aN.value}}` na interpolacao. Todos os outros steps seguem `.body`; MEMORY_STORE_GET nao. |
